...
import { Payment, SavingsGoal, CreditCard } from '../types.ts';
...
const CalendarView = ({ payments, goals, creditCards, onDayClick }: CalendarViewProps) => {
    ...
    const events = useMemo(() => {
        ...
        const cardEvents = creditCards
            .filter(card => card.currentBalance > 0)
            .flatMap(card => {
                const thisMonthDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), card.paymentDueDateDay);
                const nextMonthDueDate = new Date(nextMonthDate.getFullYear(), nextMonthDate.getMonth(), card.paymentDueDateDay);
                
                return [
                    {
                        id: `cc-${card.id}-${thisMonthDate.toISOString()}`,
                        date: thisMonthDate,
                        title: `Límite ${card.name}`,
                        color: card.color,
                        type: 'payment' as const,
                    },
                    {
                        id: `cc-${card.id}-${nextMonthDueDate.toISOString()}`,
                        date: nextMonthDueDate,
                        title: `Límite ${card.name}`,
                        color: card.color,
                        type: 'payment' as const,
                    }
        ...
        return [...paymentEvents, ...goalEvents, ...cardEvents];
    }, [payments, goals, creditCards, currentDate, nextMonthDate]);
